...
public class OBJLoader {
	public static ModelData loadOBJ(String objFileName) {
		...
		try {
			...
			/*
			 * while (line != null && line.startsWith("f ")) { String[]
			 * currentLine = line.split(" "); String[] vertex1 =
			 * currentLine[1].split("/"); String[] vertex2 =
			 * currentLine[2].split("/"); String[] vertex3 =
			 * currentLine[3].split("/"); processVertex(vertex1, vertices,
			 * indices); processVertex(vertex2, vertices, indices);
			 * processVertex(vertex3, vertices, indices); line =
			 * reader.readLine(); }
			 */
			reader.close();
		} catch (IOException e) {
	...
	private static void processVertex(String[] vertex, List<Vertex> vertices, List<Integer> indices) {
		...
		int normalIndex = (vertex[2].length() != 0) ? Integer.parseInt(vertex[2]) - 1 : 0;
	...
	private static float convertDataToArrays(List<Vertex> vertices, List<Vector2f> textures, List<Vector3f> normals,
		...
		for (int i = 0; i < vertices.size(); i++) {
			...
			Vector2f textureCoord = new Vector2f(1, 1);
